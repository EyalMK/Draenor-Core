...
#define ENTRY_JAINA_ANOMALY 94922

enum eAnomaly
{
	SPELL_TEMPORAL_POWER = 190623
};

class npc_jaina_anomaly : public CreatureScript
{
public:
	npc_jaina_anomaly() : CreatureScript("npc_jaina_anomaly") { }

	struct npc_jaina_anomalyAI : public ScriptedAI
	{
		npc_jaina_anomalyAI(Creature* pCreature) : ScriptedAI(pCreature) { }

		bool OnSummon(Player* player /*targets*/)
		{
			...
			for (Map::PlayerList::const_iterator i = _list.begin(); i != _list.end(); ++i)
			{
				if (Player* pPlayer = i->getSource())
					if (pPlayer->IsWithinDistInMap(player, 10.0f))
						pPlayer->SendPlaySound(10699, player->GetGUID());
			}
		}
		...
		void Reset()
		{
			...
			if (owner->HasAura(186165)) // T18 4P Arcane
				me->AddAura(SPELL_TEMPORAL_POWER, owner);
			...
			me->SetFullHealth();
			me->SetReactState(ReactStates::REACT_DEFENSIVE);
			me->CastSpell(me, 44816); // Transparency visual
			me->CastSpell(me, 51347); // Teleport visual
			// Inherit Master's Threat List
			...
			switch (me->GetEntry())
			{
				if (me->IsValidAttackTarget(target))
				{
					me->CastSpell(target, 191764, false); // Frostbolt
					AttackStart(target);
				}
			}
			...
			{
				WeaponAttackType l_AttackTypeEnum = static_cast<WeaponAttackType>(l_AttackType);
				me->SetBaseWeaponDamage(l_AttackTypeEnum, WeaponDamageRange::MAXDAMAGE, owner->GetWeaponDamageRange(l_AttackTypeEnum, WeaponDamageRange::MAXDAMAGE));
				me->SetBaseWeaponDamage(l_AttackTypeEnum, WeaponDamageRange::MINDAMAGE, owner->GetWeaponDamageRange(l_AttackTypeEnum, WeaponDamageRange::MINDAMAGE));
			}
	...
	CreatureAI* GetAI(Creature* pCreature) const
	{
		return new npc_jaina_anomalyAI(pCreature);
	}
};

/*######
## npc_sylvanas_anomaly
######*/

#define ENTRY_SYLVANAS_ANOMALY 94925

class npc_sylvanas_anomaly : public CreatureScript
{
public:
	npc_sylvanas_anomaly() : CreatureScript("npc_sylvanas_anomaly") { }

	struct npc_sylvanas_anomalyAI : public ScriptedAI
	{
		npc_sylvanas_anomalyAI(Creature* pCreature) : ScriptedAI(pCreature) { }
		...
		bool OnSummon(Player* player /*targets*/)
		{
			...
			for (Map::PlayerList::const_iterator i = _list.begin(); i != _list.end(); ++i)
			{
				if (Player* pPlayer = i->getSource())
					if (pPlayer->IsWithinDistInMap(player, 10.0f))
						pPlayer->SendPlaySound(7223, player->GetGUID());
			}
		}

		void Reset(const uint32 diff)
		{
			Unit* owner = me->ToTempSummon() ? me->ToTempSummon()->GetSummoner() : NULL;
			Unit* target = owner ? (owner->getVictim() ? owner->getVictim() : (owner->ToPlayer() ? owner->ToPlayer()->GetSelectedUnit() : NULL)) : NULL;
			...
			switch (me->GetEntry())
			{
				if (me->IsValidAttackTarget(target))
				{
					me->CastSpell(target, 191799, false); // Shoot
					AttackStart(target);
				}
			}
	...
	CreatureAI* GetAI(Creature* pCreature) const
	{
		return new npc_sylvanas_anomalyAI(pCreature);
	}
};
...
#define ENTRY_ARTHAS_ANOMALY 94946

class npc_arthas_anomaly : public CreatureScript
{
public:
	npc_arthas_anomaly() : CreatureScript("npc_arthas_anomaly") { }

	struct npc_arthas_anomalyAI : public ScriptedAI
	{
		npc_arthas_anomalyAI(Creature* pCreature) : ScriptedAI(pCreature) { }
		...
		bool OnSummon(Player* player /*targets*/)
		{
			...
			for (Map::PlayerList::const_iterator i = _list.begin(); i != _list.end(); ++i)
			{
				if (Player* pPlayer = i->getSource())
					if (pPlayer->IsWithinDistInMap(player, 10.0f))
						pPlayer->SendPlaySound(14326, player->GetGUID());
			}
		}
	...
	CreatureAI* GetAI(Creature* pCreature) const
	{
		return new npc_arthas_anomalyAI(pCreature);
	}
};
...
#define ENTRY_TYRANDE_ANOMALY 94879

class npc_tyrande_anomaly : public CreatureScript
{
public:
	npc_tyrande_anomaly() : CreatureScript("npc_tyrande_anomaly") { }

	struct npc_tyrande_anomalyAI : public ScriptedAI
	{
		npc_tyrande_anomalyAI(Creature* pCreature) : ScriptedAI(pCreature) { }
		...
		bool OnSummon(Player* player /*targets*/)
		{
			...
			for (Map::PlayerList::const_iterator i = _list.begin(); i != _list.end(); ++i)
			{
				if (Player* pPlayer = i->getSource())
					if (pPlayer->IsWithinDistInMap(player, 10.0f))
						pPlayer->SendPlaySound(114685, player->GetGUID());
			}
		}
	...
	CreatureAI* GetAI(Creature* pCreature) const
	{
		return new npc_tyrande_anomalyAI(pCreature);
	}
};
...
void AddSC_npcs_special()
{
	...
	new npc_jaina_anomaly();
	new npc_sylvanas_anomaly();
	new npc_arthas_anomaly();
	new npc_tyrande_anomaly();
    new npc_luo_meng();
    new npc_monk_spirit();
    new npc_rogue_decoy();
