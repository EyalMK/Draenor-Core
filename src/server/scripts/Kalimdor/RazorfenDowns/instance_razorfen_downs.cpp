            ...
            switch (gameObject->GetEntry())
            {
                case GO_GONG:
                    ...
                    if (GetBossState(DATA_TUTEN_KASH) == DONE)
                        gameObject->SetFlag(UNIT_FIELD_FLAGS, GO_FLAG_NOT_SELECTABLE);
                    break;
                ...
                switch (data)
                {
                    case IN_PROGRESS:
                    {
                        if (GameObject* go = instance->GetGameObject(goGongGUID))
                            go->SetFlag(UNIT_FIELD_FLAGS, GO_FLAG_NOT_SELECTABLE);
                    ...
                    case NPC_TOMB_FIEND:
                        if (++fiendsKilled == 10)
                        {
                            fiendsKilled = 0;
                            if (GameObject* go = instance->GetGameObject(goGongGUID))
                                go->RemoveFlag(UNIT_FIELD_FLAGS, GO_FLAG_NOT_SELECTABLE);
                        }
                        break;
