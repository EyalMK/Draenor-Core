	...
	class spell_kabummbombSpellScript : public SpellScript
	{
		...
		void HandleActivateGameobject(SpellEffIndex effIndex)
		{
			if (Unit* caster = GetCastItem()->GetOwner())
			{
				...
				{
//					caster->SummonGameObject(GO_DEPOSIT_CHUNK, Deposit->GetPositionX() - 1, Deposit->GetPositionY(), Deposit->GetPositionZ() + 1, 0, 0);
//					caster->SummonGameObject(GO_DEPOSIT_CHUNK, Deposit->GetPositionX() - 1, Deposit->GetPositionY() - 1, Deposit->GetPositionZ() + 1, 0, 0);
//					caster->SummonGameObject(GO_DEPOSIT_CHUNK, Deposit->GetPositionX() - 1, Deposit->GetPositionY() + 1, Deposit->GetPositionZ() + 1, 0, 0);
					Deposit->Delete();
				}
			}
...
//					me->SummonGameObject(GO_DEPOSIT, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, 120000);
				_t = 60000;
			}
			else _t -= diff;
		}
