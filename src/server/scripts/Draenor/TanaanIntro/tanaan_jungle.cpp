...
/*  Khadgar's Arcane Aegis  -> 165667 
	Flying capsules			-> 165666
	Vengeance of Earth (thrall) -> 165843 
	164677 - thaelin / 167689 - hansel

	164024 -> warsong banner
...
class playerScript_enter_tanaan : public PlayerScript
{
    public:
		...
		{
			if (p_Player->GetZoneId() == 7025) // Tanaan Intro
			{
				...
				{
					l_PhaseMask == 31; // To avoid weird bugs we ensure the phases are correct at the starting of the questline
				}

				if (p_Player->GetQuestStatus(TanaanQuests::QuestOnslaughtEnd) == QUEST_STATUS_INCOMPLETE)
				{
					if (!p_Player->GetQuestObjectiveCounter(TanaanQuestObjectives::ObjNorthernSpireDisabled))
					{
						p_Player->PlayScene(TanaanSceneObjects::SceneLightingSpireNorth, p_Player);

						l_PhaseMask |= TanaanPhases::PhaseChoGall;
					}

					if (!p_Player->GetQuestObjectiveCounter(TanaanQuestObjectives::ObjSouthernSpireDisabled))
					{
						p_Player->PlayScene(TanaanSceneObjects::SceneLightingSpireSouth, p_Player);
        ...
        void OnUpdateZone(Player* p_Player, uint32 p_NewZoneId, uint32 p_OldZoneID, uint32 p_NewAreaId) override
        {
			...
			if (p_OldZoneID != TanaanZones::ZoneTanaanJungle)
			{
				...
				p_Player->PlayScene(TanaanSceneObjects::SceneLoomingArmy, p_Player);
			}
        }

		void OnUpdateArea(Player* p_Player, uint32 p_NewAreaId, uint32 /*oldAreaID*/)
		{
			switch (p_NewAreaId)
			{
				...
				{
					p_Player->PlayScene(TanaanSceneObjects::SceneCuriousHatchets, p_Player);
					break;
				}
				case TanaanZones::AreaBleedingAltar:
				{
					p_Player->PlayScene(TanaanSceneObjects::SceneEyeOfKilrogg, p_Player);
					p_Player->PlayScene(TanaanSceneObjects::SceneBloodBowl, p_Player);
					break;
				}
				case TanaanZones::AreaUmbralHalls:
				{
					p_Player->PlayScene(TanaanSceneObjects::SceneUmbralHallsPortals, p_Player);
					p_Player->PlayScene(TanaanSceneObjects::SceneKelidanVisual1, p_Player);
					p_Player->PlayScene(TanaanSceneObjects::SceneKelidanVisual2, p_Player);
					break;
				}
				...
				case TanaanZones::AreaBlackRockQuarry:
				{
					...
					{
						p_Player->PlayScene(TanaanSceneObjects::SceneKhadgarAtDam, p_Player);
...
class playerScript_add_tanaan_for_max_level : public PlayerScript
{
    public:
		...
		{
			if (p_Player->getLevel() && p_OldLevel == 89)
			{
				if (const Quest* l_StartDraenor = sObjectMgr->GetQuestTemplate(TanaanQuests::QuestStartDraenor))
					p_Player->AddQuest(l_StartDraenor, nullptr);
			}
...
class map_dark_portal_entrance : public WorldMapScript
{
    public:
		...
		{ 
			p_Map->SetObjectVisibility(350.0f); 
		}
};
...
public:
	...
	struct npc_generic_tanaan_guardianAI : public ScriptedAI
	{
		...
		void Reset() override
		{
			...
			if (me->GetEntry() == TanaanCreatures::NpcArchon)
				me->CastSpell(me, 165720, true);  // Inner Fire

			if (me->GetEntry() == TanaanCreatures::NpcBarbery)
				me->CastSpell(me, 165739, true);  // Power Word: Fortitude

			if (me->GetEntry() == TanaanCreatures::NpcNorthpaul)
				me->AddAura(165767, me);  // Shadowform

			if (me->GetEntry() == TanaanCreatures::NpcYoori)
				me->CastSpell(me, 165754, true);  // Mark of the Wild

			if (me->GetEntry() == TanaanCreatures::NpcQiana)
				me->CastSpell(me, 165754, true);  // Mark of the Wild

			if (me->GetEntry() == TanaanCreatures::NpcMaelgwyn)
				me->CastSpell(me, 165754, true);  // Mark of the Wild

			if (me->GetEntry() == TanaanCreatures::NpcAstrylian)
				me->CastSpell(me, 165754, true);  // Mark of the Wild

			if (me->GetEntry() == TanaanCreatures::NpcKengtus)
				me->CastSpell(me, 165754, true);  // Mark of the Wild

			if (me->GetEntry() == TanaanCreatures::NpcDarragh)
				me->CastSpell(me, 165754, true);  // Mark of the Wild

			if (me->GetEntry() == TanaanCreatures::NpcMoonalli)
				me->CastSpell(me, 165754, true);  // Mark of the Wild

			if (me->GetEntry() == TanaanCreatures::NpcKengtus)
				me->AddAura(24858, me);  // Moonkin Form

			// No mana cost
			switch (me->GetEntry())
			{
				case TanaanCreatures::NpcQiana:
				case TanaanCreatures::NpcShoju:
				case TanaanCreatures::NpcMonishot:
				case TanaanCreatures::NpcEd:
				case TanaanCreatures::NpcRyii:
				case TanaanCreatures::NpcLupas:
				case TanaanCreatures::NpcRacy:
					break;
				default:
					me->AddAura(83458, me);
					break;
			...
			if (me->GetEntry() == TanaanCreatures::NpcQiana)
				me->AddAura(182274, me);  // Energy

			if (me->GetEntry() == TanaanCreatures::NpcHaomi)
				me->AddAura(182274, me);  // Energy

			if (me->GetEntry() == TanaanCreatures::NpcFelguard)
				me->AddAura(182274, me);  // Energy

			if (me->GetEntry() == TanaanCreatures::NpcMonishot)
				me->AddAura(182274, me);  // Energy

			if (me->GetEntry() == TanaanCreatures::NpcEd)
				me->AddAura(182274, me);  // Energy

			if (me->GetEntry() == TanaanCreatures::NpcLupas)
				me->AddAura(182274, me);  // Energy

			if (me->GetEntry() == TanaanCreatures::NpcRyii)
				me->AddAura(182274, me);  // Energy

			if (me->GetEntry() == TanaanCreatures::NpcRacy)
				me->AddAura(182274, me);  // Energy

			if (me->GetEntry() == TanaanCreatures::NpcRainiara)
				me->AddAura(182274, me);  // Energy

			if (me->GetEntry() == TanaanCreatures::NpcRoague)
				me->AddAura(182274, me);  // Energy

			if (me->GetEntry() == TanaanCreatures::NpcFelguard)
				me->AddAura(182274, me);  // Energy

			if (me->GetEntry() == TanaanCreatures::NpcImp)
				me->AddAura(182274, me);  // Energy

			if (me->GetEntry() == TanaanCreatures::NpcAstrylian)
				me->CastSpell(me, 165771, true);  // Thorns

			if (me->GetEntry() == TanaanCreatures::NpcCorselle)
				me->CastSpell(me, 165747, true);  // Blessing of Might

			if (me->GetEntry() == TanaanCreatures::NpcCorselle)
				me->AddAura(165746, me);  // Retribution Aura

			if (me->GetEntry() == TanaanCreatures::NpcCorselle)
				me->CastSpell(me, 165745, true);  // Seal of Truth

			if (me->GetEntry() == TanaanCreatures::NpcAgios)
				me->CastSpell(me, 165713, true);  // Blessing of Kings

			if (me->GetEntry() == TanaanCreatures::NpcDurphorn)
				me->CastSpell(me, 165713, true);  // Blessing of Kings

			if (me->GetEntry() == TanaanCreatures::NpcAgios)
				me->AddAura(165712, me);  // Devotion Aura

			if (me->GetEntry() == TanaanCreatures::NpcDurphorn)
				me->AddAura(165712, me);  // Devotion Aura

			if (me->GetEntry() == TanaanCreatures::NpcFutz)
				me->CastSpell(me, 165723, true);  // Battle Shout

			if (me->GetEntry() == TanaanCreatures::NpcAriokVanguard)
				me->CastSpell(me, 165723, true);  // Battle Shout

			if (me->GetEntry() == TanaanCreatures::NpcMumper)
				me->CastSpell(me, 165723, true);  // Battle Shout

			if (me->GetEntry() == TanaanCreatures::NpcElaynea)
				me->CastSpell(me, 165723, true);  // Battle Shout

			if (me->GetEntry() == TanaanCreatures::NpcFutz)
				me->CastSpell(me, 122990, true);  // Battle Stance

			if (me->GetEntry() == TanaanCreatures::NpcEtubrute)
				me->CastSpell(me, 122990, true);  // Battle Stance
			...
			if (me->GetEntry() == TanaanCreatures::NpcOlin)
				me->CastSpell(me, 165750, true);  // Commanding Shout

			if (me->GetEntry() == TanaanCreatures::NpcEtubrute)
				me->CastSpell(me, 165750, true);  // Commanding Shout

			if (me->GetEntry() == TanaanCreatures::NpcOlin)
				me->CastSpell(me, 41101, true);  // Defensive Stance

			if (me->GetEntry() == TanaanCreatures::NpcTurkina)
				me->CastSpell(me, 165762, true);  // Horn of Winter

			if (me->GetEntry() == TanaanCreatures::NpcMoriccalas)
				me->CastSpell(me, 165762, true);  // Horn of Winter

			if (me->GetEntry() == TanaanCreatures::NpcTore)
				me->CastSpell(me, 165762, true);  // Horn of Winter

			if (me->GetEntry() == TanaanCreatures::NpcBonesaw)
				me->CastSpell(me, 165762, true);  // Horn of Winter

			if (me->GetEntry() == TanaanCreatures::NpcTurkina)
				me->CastSpell(me, 48265, true);   // Unholy Presence

			if (me->GetEntry() == TanaanCreatures::NpcMoriccalas)
				me->CastSpell(me, 48263, true);   // Blood Presence

			if (me->GetEntry() == TanaanCreatures::NpcTore)
				me->CastSpell(me, 48266, true);   // Frost Presence

			if (me->GetEntry() == TanaanCreatures::NpcBonesaw)
				me->CastSpell(me, 48266, true);   // Frost Presence

			if (me->GetEntry() == TanaanCreatures::NpcAnruin)
				me->CastSpell(me, 165735, true);  // Fel Armor

			if (me->GetEntry() == TanaanCreatures::NpcNevo)
				me->CastSpell(me, 165735, true);  // Fel Armor

			if (me->GetEntry() == TanaanCreatures::NpcChalle)
				me->CastSpell(me, 165743, true);  // Frost Armor

			if (me->GetEntry() == TanaanCreatures::NpcMalothas)
				me->CastSpell(me, 79849, true);   // Molten Armor	

			if (me->GetEntry() == TanaanCreatures::NpcAlya)
				me->CastSpell(me, 164309, true);  // Mage Armor

			if (me->GetEntry() == TanaanCreatures::NpcThrend)
				me->CastSpell(me, 165773, true);  // Earth Shield

			if (me->GetEntry() == TanaanCreatures::NpcMiserain)
				me->CastSpell(me, 115921, true);  // Legacy of the Emperor
			...
			switch (me->GetEntry())
			{
				case TanaanCreatures::NpcAlya:
				case TanaanCreatures::NpcArchon:
					enemyId = TanaanCreatures::NpcShadowRitualist;
					break;
				default:
					enemyId = TanaanCreatures::NpcIronGrunt;
					break;
			}
			...
			switch (me->GetEntry())
			{
				case TanaanCreatures::NpcBonesaw || TanaanCreatures::NpcHaomi || TanaanCreatures::NpcDarragh || TanaanCreatures::NpcAstrylian || TanaanCreatures::NpcArnold || TanaanCreatures::NpcAgios:
					enemyNumber = 3;
					break;
				case TanaanCreatures::NpcThrend:
					enemyNumber = 1;
					break;
				case TanaanCreatures::NpcEtubrute:
					enemyNumber = 1;
					break;
				case TanaanCreatures::NpcQiana:
					enemyNumber = 1;
					break;
				case TanaanCreatures::NpcAriokVanguard:
					enemyNumber = 1;
					break;
				case TanaanCreatures::NpcShoju:
					enemyNumber = 1;
				...
				case TanaanCreatures::NpcRacy:
					enemyNumber = 1;
					break;
				case TanaanCreatures::NpcNorthpaul:
					enemyNumber = 1;
					break;
				case TanaanCreatures::NpcRainiara:
					enemyNumber = 0;
					break;
				default:
					enemyNumber = 2;
			...
			events.ScheduleEvent(EVENT_CHECK_ENEMY, 1000);
			events.ScheduleEvent(EVENT_RESTORE_HEALTH, 10000);

			// Casters should use spells more often
			switch (me->GetEntry())
			{
				case TanaanCreatures::NpcChalle:
				case TanaanCreatures::NpcArchon:
				case TanaanCreatures::NpcKengtus:
				case TanaanCreatures::NpcMoonalli:
				case TanaanCreatures::NpcYoori:
				case TanaanCreatures::NpcNevo:
				case TanaanCreatures::NpcAnruin:
				case TanaanCreatures::NpcBarbery:
				case TanaanCreatures::NpcAlya:
				case TanaanCreatures::NpcDarragh:
				case TanaanCreatures::NpcThrend:
				case TanaanCreatures::NpcNorthpaul:
					events.ScheduleEvent(EVENT_RANDOM_SPELL, urand(4000, 5000));
					break;
				case TanaanCreatures::NpcEd:
				case TanaanCreatures::NpcMonishot:
				case TanaanCreatures::NpcRainiara:
					events.ScheduleEvent(EVENT_RANDOM_SPELL, urand(1500, 2500));
					break;
				default:
					events.ScheduleEvent(EVENT_RANDOM_SPELL, urand(6000, 8000));
					break;
			}
		}
		...
		{
			return me->GetEntry() == TanaanCreatures::NpcMumper || me->GetEntry() == TanaanCreatures::NpcMoriccalas;
		}

		void SpawnEnemy()
		{
			enemySpawnPos = me->GetHomePosition();
			...
			if (enemyId == TanaanCreatures::NpcShadowRitualist)
				GetRandPosFromCenterInDist(me, frand(3.0f, 7.0f), enemySpawnPos);
			else
				GetPositionWithDistInOrientation(me, 2.0f, me->GetOrientation() + frand(-3.0f, 3.0f), enemySpawnPos);

			if (TempSummon* enemy = me->SummonCreature(enemyId, enemySpawnPos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000))
			{
				if (enemyId == TanaanCreatures::NpcShadowRitualist)
					enemy->CastSpell(enemy, SPELL_RITUALIST_SPAWN, true);

				//ennemy->SetPlayerDamageReq(ennemy->GetHealth() / 10);
				enemy->Attack(me, enemyId != TanaanCreatures::NpcShadowRitualist);
			}
		}
		...
		void SummonedCreatureDespawn(Creature* summon) override
		{
			...
			events.ScheduleEvent(EVENT_CHECK_ENEMY, urand(0, 10000));
		}
		...
		void EnterCombat(Unit* p_Attacker) override
		{
			switch (me->GetEntry())
			{
				case TanaanCreatures::NpcRoague:
				case TanaanCreatures::NpcLupas:
				case TanaanCreatures::NpcRyii:
				case TanaanCreatures::NpcRacy:
				case TanaanCreatures::NpcShoju:
					me->SetAttackTime(WeaponAttackType::BaseAttack, 1200 + urand(0, 450));
					break;
				case TanaanCreatures::NpcEd:
				case TanaanCreatures::NpcRainiara:
					me->SetAttackTime(WeaponAttackType::RangedAttack, 1200 + urand(0, 450));
					!me->haveOffhandWeapon();
					break;
				default:
					me->SetAttackTime(WeaponAttackType::RangedAttack, 2000 + urand(0, 450));
					!me->haveOffhandWeapon();
					break;
			}
		}
		...
		void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				switch (eventId)
				{
					case EVENT_CHECK_ENEMY:
					{
						uint8 currentEnemyNumber = summons.size();
					...
					case EVENT_RANDOM_SPELL:
					{
						...
						if (spellId == 165752 || 165753 || 165772 || 165718 || 165737 || 165510 || 165131 || 165129
							|| 165128) // Healing Spells
						{
						...
						switch (me->GetEntry())
						{
							...
							case TanaanCreatures::NpcMonishot:
							case TanaanCreatures::NpcRainiara:
								events.ScheduleEvent(EVENT_RANDOM_SPELL, urand(500, 1500));
								break;
							default:
								events.ScheduleEvent(EVENT_RANDOM_SPELL, urand(6000, 8000));
								break;
        ...
        struct npc_iron_gruntAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                ...
                else if (this->CheckPosition(eCreatureIds::Mumper))
                {
                    ...
                    me->GetMotionMaster()->MoveCharge(4037.468750f, -2318.861816f, 64.682114f, 10.0f, 1);
                    me->SetReactState(REACT_PASSIVE);
                    me->SetUInt32Value(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_NPC);
                }
                ...
                switch (p_Id)
                {
                    ...
                    case eCreatureIds::Mumper:
                    {
                        if ((m_Pos.m_positionX <= 4043.0f && m_Pos.m_positionX >= 4042.0f) &&
                            (m_Pos.m_positionY >= -2327.0f && m_Pos.m_positionY <= -2326.0f))
                            return true;
                    }
                    default:
                        break;
            ...
            void MovementInform(uint32 p_Type, uint32 p_Id) override
            {
                ...
                if (this->CheckPosition(eCreatureIds::Moriccalas))
                {
                    switch (p_Id)
                    {
                        case 1:
                            ...
                            break;
                        case 2:
                            me->GetMotionMaster()->MoveCharge(4078.851563f, -2343.972656f, 76.311646f, 10.0f, 3);
                            break;
                        case 3:
                        {
                ...
                else if (this->CheckPosition(eCreatureIds::Mumper))
                {
                    ...
                    {
						case 1:
							me->GetMotionMaster()->MoveCharge(4053.675293f, -2318.949463f, 64.685486f, 10.0f, 2);
							break;
                        case 2:
                            me->GetMotionMaster()->MoveCharge(4054.161377f, -2358.301025f, 85.735275f, 10.0f, 3);
                            break;
                        case 3:
                        {
                            if (me->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_NPC))
...
/// 300007 - Thaelin Trigger (Friendly)
class npc_thaelin_trigger : public CreatureScript
{
public:
	npc_thaelin_trigger() : CreatureScript("npc_thaelin_trigger") { }

	CreatureAI* GetAI(Creature* creature) const
	{
		return new npc_thaelin_triggerAI(creature);
	}

	struct npc_thaelin_triggerAI : public ScriptedAI
	{
		npc_thaelin_triggerAI(Creature* creature) : ScriptedAI(creature) { }
		...
		void MoveInLineOfSight(Unit* p_Unit) override
		{
			...
			if (l_Player->GetQuestStatus(TanaanQuests::QuestOnslaughtEnd) == QUEST_STATUS_INCOMPLETE && l_Player->GetQuestObjectiveCounter(TanaanQuestObjectives::ObjSouthernSpireDisabled) < 1)
				l_Player->CastSpell(l_Player, 164677, true); // Thaelin Trigger Scene
		}
	};
};

/// 300008 - Hansel Trigger (Friendly)
class npc_hansel_trigger : public CreatureScript
{
public:
	npc_hansel_trigger() : CreatureScript("npc_hansel_trigger") { }

	CreatureAI* GetAI(Creature* creature) const
	{
		return new npc_hansel_triggerAI(creature);
	}

	struct npc_hansel_triggerAI : public ScriptedAI
	{
		npc_hansel_triggerAI(Creature* creature) : ScriptedAI(creature) { }
		...
		void MoveInLineOfSight(Unit* p_Unit) override
		{
			...
			if (l_Player->GetQuestStatus(TanaanQuests::QuestOnslaughtEnd) == QUEST_STATUS_INCOMPLETE && l_Player->GetQuestObjectiveCounter(TanaanQuestObjectives::ObjNorthernSpireDisabled) < 1)
				l_Player->CastSpell(l_Player, 167689, true); // Hansel Trigger Scene
		}
	};
};

/// 81695 - Cho'gall
class npc_cho_gall_trigger : public CreatureScript
{
public:
	npc_cho_gall_trigger() : CreatureScript("npc_cho_gall_trigger") { }

	bool m_beenCastOnCho;

	struct npc_cho_gall_triggerAI : public ScriptedAI
	{
		npc_cho_gall_triggerAI(Creature* creature) : ScriptedAI(creature) { }
		...
		void MoveInLineOfSight(Unit* p_Unit) override
		{
			...
			if (l_Player->GetQuestStatus(TanaanQuests::QuestOnslaughtEnd) == QUEST_STATUS_INCOMPLETE && l_Player->GetQuestObjectiveCounter(TanaanQuestObjectives::ObjNorthernSpireDisabled) < 1)
				me->AI()->Talk(0);
			p_Unit->ToPlayer()->SetCastOnTeron(true);
		}
	};

	CreatureAI* GetAI(Creature* creature) const
	{
		return new npc_cho_gall_triggerAI(creature);
	}
};

/// 81696 - Teron'gor
class npc_teron_gor_trigger : public CreatureScript
{
public:
	npc_teron_gor_trigger() : CreatureScript("npc_teron_gor_trigger") { }
	...
	struct npc_teron_gor_triggerAI : public ScriptedAI
	{
		npc_teron_gor_triggerAI(Creature* creature) : ScriptedAI(creature) { }
		...
		void MoveInLineOfSight(Unit* p_Unit) override
		{
			...
			if (l_Player->GetQuestStatus(TanaanQuests::QuestOnslaughtEnd) == QUEST_STATUS_INCOMPLETE && l_Player->GetQuestObjectiveCounter(TanaanQuestObjectives::ObjSouthernSpireDisabled) < 1)
				me->AI()->DoAction(eActions::StartDialogue);
		}
	...
	CreatureAI* GetAI(Creature* creature) const
	{
		return new npc_teron_gor_triggerAI(creature);
	}

};

/// 82647 - Tormented Soul (enemy)
...
/// Any object that is part of a new phase and needs to be destroyed
class go_platform_tanaan_destroyed : public GameObjectScript
{
public:
	go_platform_tanaan_destroyed() : GameObjectScript("go_platform_tanaan") { }

	struct go_platform_tanaan_destroyedAI : public GameObjectAI
	{
		go_platform_tanaan_destroyedAI(GameObject* p_Go) : GameObjectAI(p_Go) { }
	...
	GameObjectAI* GetAI(GameObject* p_Go) const
	{
		return new go_platform_tanaan_destroyedAI(p_Go);
	}
};
        ...
        bool OnGossipHello(Player* p_Player, GameObject* p_Gameobject) override
        {
            if (p_Player->GetQuestStatus(TanaanQuests::QuestOnslaughtEnd) == QUEST_STATUS_INCOMPLETE)
            {
                ...
                if (p_Gameobject->GetEntry() == TanaanGameObjects::GobMarkOfShadowmoon)
                {
					...
					p_Player->CancelStandaloneScene(TanaanSceneObjects::SceneLightingSpireNorth);
                ...
                else if (p_Gameobject->GetEntry() == TanaanGameObjects::GobMarkOfBleedingHollow)
                {
					...
					p_Player->CancelStandaloneScene(TanaanSceneObjects::SceneLightingSpireSouth);
...
void AddSC_tanaan_jungle()
{
	...
	new npc_hansel_trigger();
	new npc_thaelin_trigger();
	...
	new npc_cho_gall_trigger();
	new npc_teron_gor_trigger();
    new gob_static_rune();
    new go_platform_tanaan();
	new go_platform_tanaan_destroyed();
