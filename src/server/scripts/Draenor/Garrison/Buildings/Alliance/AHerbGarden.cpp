...
namespace MS {
	namespace Garrison
	{
		...
		{
			me->DespawnCreaturesInArea(NPCs::NpcLunarfallRaccoon, 200.0f);
		}
		...
		void npc_NaronBloomthistleAI::OnSetPlotInstanceID(uint32 p_PlotInstanceID)
		{
			...
			if (!l_Owner->IsQuestRewarded(Quests::Alliance_ClearingTheGarden))
			{
				...
				me->GetCreatureListWithEntryInGrid(l_CreatureList, NPCs::NpcLunarfallRaccoon, 65.0f);
				...
				for (uint32 l_I = 0; l_I < s_PositionCount; ++l_I)
				{
					...
					Creature * l_Creature = SummonRelativeCreature(NPCs::NpcLunarfallRaccoon, l_CurrentPosition.X, l_CurrentPosition.Y, l_CurrentPosition.Z, 0, TEMPSUMMON_MANUAL_DESPAWN);
					if (l_Creature)
