...
/// Constructor
    go_frostridge_master_surveyor::go_frostridge_master_surveyor()
        : GameObjectScript("go_frostridge_master_surveyor")
    {
    ...
    /// @p_Player   : Source player instance
    /// @p_Creature : Target creature instance
    bool go_frostridge_master_surveyor::OnGossipHello(Player* p_Player, GameObject* p_GameObject)
    {
        if (!p_Player->GetGarrison())
            p_Player->ADD_GOSSIP_ITEM_DB(GarrisonGossipMenus::MenuID::DefaultMenuGreetings, GarrisonGossipMenus::GossipOption::GarrisonCreation, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);

        p_Player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, p_GameObject->GetGUID());

        return true;
    }

bool go_frostridge_master_surveyor::OnGossipSelect(Player* p_Player, GameObject* p_GameObject, uint32 /*p_Sender*/, uint32 /*p_Action*/)
    {
        if (p_Player->getLevel() >= 88 && !p_Player->GetGarrison())
        {
            p_Player->CLOSE_GOSSIP_MENU();
            ...
            if (l_TeamID == TEAM_ALLIANCE && p_Player->GetQuestStatus(Quests::QUEST_ETABLISH_YOUR_GARRISON_A) != QUEST_STATUS_REWARDED)
            {
                p_Player->AddQuest(sObjectMgr->GetQuestTemplate(Quests::QUEST_ETABLISH_YOUR_GARRISON_A), p_GameObject);
                p_Player->CompleteQuest(Quests::QUEST_ETABLISH_YOUR_GARRISON_A);
            }
            else if (l_TeamID == TEAM_HORDE && p_Player->GetQuestStatus(Quests::QUEST_ETABLISH_YOUR_GARRISON_H) != QUEST_STATUS_REWARDED)
            {
                p_Player->AddQuest(sObjectMgr->GetQuestTemplate(Quests::QUEST_ETABLISH_YOUR_GARRISON_H), p_GameObject);
                p_Player->CompleteQuest(Quests::QUEST_ETABLISH_YOUR_GARRISON_H);
            }
...
void AddSC_Garrison_NPC()
{
    ...
    new MS::Garrison::go_frostridge_master_surveyor;
