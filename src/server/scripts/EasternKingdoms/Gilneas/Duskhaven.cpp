...
enum eDuskHaven
{
	...
	NPC_LORD_DARIUS_CROWLEY_DUSKHAVEN_2			= 37195,
...
public:
    ...
    void OnQuestUpdate(Player* player, uint32 questId)
    {
...
public:
    ...
    bool OnQuestReward(Player* player, Creature* /*creature*/, const Quest* quest, uint32) override
    {
        if (quest->GetQuestId() == QUEST_LAST_CHANCE_AT_HUMANITY)
        {
            ...
            player->SetFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_REGENERATE_POWER);
                ...
                switch (eventId)
                {
                    ...
                    case EVENT_TALK_PART_00:
                    {
                        ...
                        if (Creature* king = Unit::GetCreature(*me, m_kingGUID))
                            king->RemoveFlag(UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_QUESTGIVER);
                    ...
                    {
                        if (Player* player = Unit::GetPlayer(*me, m_playerGUID))
                            if (Creature* lord = Unit::GetCreature(*me, m_godfreyGUID))
                            {
                    ...
                    case EVENT_TALK_PART_04:
                    {
                        ...
                        if (Creature* king = Unit::GetCreature(*me, m_kingGUID))
                            king->SetFlag(UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_QUESTGIVER);
        ...
        void CheckVideoMember()
        {
            ...
            if (Player* player = me->SelectNearestPlayer(10.0f))
                if (!HasPlayer(player->GetGUID()))
                    if (player->GetQuestStatus(QuestIds::QUEST_LAST_STAND) == QUEST_STATUS_REWARDED && player->GetQuestStatus(QUEST_LAST_CHANCE_AT_HUMANITY) == QUEST_STATUS_NONE)
                        if (player->GetAreaId() == ZONE_DUSKHAVEN) // Check Duskhaven's AreaID
                        {
    ...
    struct npc_forsaken_catapult_36283AI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = m_events.ExecuteEvent())
            {
                ...
                {
                    if (Creature* target = Unit::GetCreature(*me, m_forsakenGUID))
                ...
                case EVENT_MASTER_RESET:
                {
                    ...
                    else
                    {
						Position pos;
						me->GetPosition(&pos);
                        if (TempSummon* npc = me->SummonCreature(NPC_FORSAKEN_MACHINIST, pos))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = m_events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    {
                        if (Creature* thyala = Unit::GetCreature(*me, m_thyalaGUID))
                        {
                            if (!thyala->isAlive() || !thyala->IsInWorld())
                            {
                                if (Player* player = Unit::GetPlayer(*me, m_player_GUID))
                    ...
                    {
                        if (Unit::GetCreature(*me, m_thyalaGUID))
...
public:
    ...
    struct npc_mastiff_36405AI : public ScriptedAI
    {
        ...
        {
            Creature* thyala = Unit::GetCreature(*me, m_thyalaGUID);
            if (!thyala)
                return;
            ...
            while (uint32 eventId = m_events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_CHECK_PLAYER:
                    {
                        ...
                        else if (Player* player = Unit::GetPlayer(*me, m_playerGUID))
                        {
    ...
    struct npc_mountain_horse_36555AI : public ScriptedAI
    {
        ...
        void IsSummonedBy(Unit* summoner) override
        {
            if (Player* player = summoner->ToPlayer())
            {
				...
				Position m_oldPosition;
				player->GetPosition(&m_oldPosition);
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        {
			Position l_Pos;
			creature->GetPosition(&l_Pos);
            player->CastSpell(player, SPELL_PHASE_QUEST_ZONE_SPECIFIC_19);
            player->SummonCreature(NPC_HARNESS_43336, l_Pos, TEMPSUMMON_MANUAL_DESPAWN);
        }
...
public:
    ...
    struct npc_harness_43336AI : public ScriptedAI
    {
        ...
        void IsSummonedBy(Unit* summoner) override
        {
            ...
            {
                m_playerGUID = player->GetGUID();
                if (Creature* car = Unit::GetCreature(*me, m_carriageGUID))
                {
...
public:
    ...
    struct npc_stagecoach_carriage_43337AI : public ScriptedAI
    {
        ...
        void PassengerBoarded(Unit* passenger, int8 /*seatId*/, bool apply) override
        {
            ...
            {
                if (passenger->ToPlayer())
                    if (Creature* car = Unit::GetCreature(*me, m_harnessGUID))
...
public:
    ...
    struct go_kings_gate_196412AI : public GameObjectAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (!m_isUsed)
                for (auto guid : cList)
                    if (Creature* horse = Unit::GetCreature(*go, guid))
                        if (horse->GetDistance(go) < 14.0f)
                        {
                            go->UseDoorOrButton(5000);
                            m_isUsed = true;
        ...
        Player* GetPlayer(Unit* mount)
        {
            if (Unit* unit = mount->GetCharmerOrOwner())
                if (Player* player = unit->ToPlayer())
                    return player;
            if (Player* player = Unit::GetPlayer(*go, mount->GetAI()->GetGUID(PLAYER_GUID)))
                return player;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = m_events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    {
                        if (Player* player = Unit::GetPlayer(*me, m_playerGUID))
                            if (Creature* koroth = Unit::GetCreature(*me, m_korothGUID))
                            {
                                koroth->AI()->Talk(0);
                                Talk(0);
                            }
    ...
    struct go_koroths_banner_201594AI : public GameObjectAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = m_events.ExecuteEvent())
            {
                ...
                case EVENT_TALK_PART_00:
                {
                    if (Creature* koroth = Unit::GetCreature(*go, m_korothGUID))
                        if (Player* player = Unit::GetPlayer(*go, m_playerGUID))
                            koroth->AI()->Talk(0);
...
public:
    ...
    bool OnUse(Player* player, Item* /*item*/, SpellCastTargets const& /*targets*/, ObjectGuid /*castId*/)
    {
        if (player->GetQuestStatus(QUEST_LOSING_YOUR_TAIL) == QUEST_STATUS_INCOMPLETE)
            if (player->HasAura(SPELL_FREEZING_TRAP_EFFECT))
...
public:
    ...
    struct npc_tobias_mistmantle_38051AI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            m_events.Reset();
            m_dariusGUID = 0;
            if (Creature* darius = me->FindNearestCreature(NPC_LORD_DARIUS_CROWLEY_DUSKHAVEN_2, 50.0f))
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = m_events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    {
                        if (Creature* godfrey = Unit::GetCreature(*me, m_godfreyGUID))
                            godfrey->AI()->Talk(0);
                    ...
                    case EVENT_START_ANIM + 3:
                    {
                        if (Creature* godfrey = Unit::GetCreature(*me, m_godfreyGUID))
                        {
                            godfrey->GetMotionMaster()->MovePath(godfrey->GetGUIDLow(), false);
