...
/*													DISABLED UNTIL CONVERTED TO WOD																	*/
/*
// 38507
...
public:
    ...
    struct npc_tobias_mistmantle_38507AI : public ScriptedAI
    {
        ...
        uint64 GetGUID(int32 id) const
        {
            switch (id)
            {
                case NPC_TOBIAS_MISTMANTLE_BFG:
                    return me->GetGUID();
                case NPC_LADY_SYLVANAS_WINDRUNNER_38530:
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = m_events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    {
                        if (Creature* tobias = Unit::GetCreature(*me, m_tobiasGUID))
                        {
                        ...
                        if (Creature* warhowl = Unit::GetCreature(*me, m_warhowlGUID))
                            warhowl->AI()->Talk(0);
                    ...
                    case EVENT_TALK_7:
                    {
                        if (Creature* crenshaw = Unit::GetCreature(*me, m_crenshawGUID))
                        {
                            crenshaw->AI()->Talk(0);
...
public:
    ...
    struct npc_tobias_mistmantle_43749AI : public ScriptedAI
    {
        ...
        {
            m_hippoList.clear();
			std::list<Creature*> hippoList;
			me->FindNearestCreature(NPC_HIPPOGRYPH, 25.0f);
            for (std::list<Creature*>::iterator itr = hippoList.begin(); itr != hippoList.end(); ++itr)
                m_hippoList.push_back((*itr));
        }
        ...
        void SetHippoSpellClick(bool value)
        {
            ...
            {
				std::list<Creature*> hippoList;
				me->FindNearestCreature(NPC_HIPPOGRYPH, 15.0f);
                for (std::list<Creature*>::const_iterator itr = hippoList.begin(); itr != hippoList.end(); ++itr)
                    if (Creature* hippo = (*itr))
                        m_hippoList.push_back(hippo);
            }

            for (std::list<Creature*>::iterator itr = m_hippoList.begin(); itr != m_hippoList.end(); ++itr)
                if (Creature* hippo = ObjectAccessor::GetCreature(*me, (*itr)->GetGUID()))
                {
                    if (value)
                        hippo->SetFlag(UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_SPELLCLICK);
                    else
                        hippo->RemoveFlag(UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_SPELLCLICK);
                }
        }
        ...
        {
        public:
            GunshipSearchWorker(uint64* gunshipGuid) : m_gunshipGuid(gunshipGuid) { }
        ...
        bool CheckIsGunshipReady()
        {
            if (GameObject* ship = ObjectAccessor::GetGameObject(*me, m_gunshipGUID))
                ...
                {
                    uint32 count = transport->GetPassengers().size();
    ...
    struct npc_hippogryph_43751AI : public ScriptedAI
    {
      ...
      // void PassengerBoarded(Unit* who, int8 /*seatId*/, bool apply) override
       /* {
        ...
        void FindLornaGuid()
        {
            if (Unit::GetCreature(*me, m_lornaGUID))
                return;
...
public:
    ...
    struct go_worgen_gunship_203428AI : public GameObjectAI
    {
        ...
        {
            if (Creature* tobias = Unit::GetCreature(*go, m_tobiasGUID))
                tobias->AI()->DoAction(eventId);

            if (Creature* trigger = Unit::GetCreature(*go, m_triggerGUID))
            ...
            {
                WorldLocation wLoc = WorldLocation(654, -1317.54f, 2121.90f, 5.6296f, 0.218f); // Hackfix supposed to leave on a wyvern
				std::list<Player*> pList;
				go->GetPlayerListInGrid(pList, 70.0f); // To test, may be buggy used to be pList = go->SelectNearestPlayer(70.0f);
...
public:
    ...
    struct npc_lorna_crowley_43566AI : public ScriptedAI
    {
      ...
      /*  {
		...
		*/
        // void EnterEvadeMode(/*reason*/) override
       /* {
            me->CombatStop(true);
            me->SetReactState(REACT_DEFENSIVE);
			...
			{
				Position l_Pos;
				me->GetTransport->GetPosition(&l_Pos);
				me->SetTransportHomePosition(l_Pos);
			}	
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (m_animStarted && pList.size())
                for (auto guid : pList)
                    if (Player* player = Unit::GetPlayer(*me, guid))
                    {
                        ...
                        {
							Position pos;
							me->GetTransport()->GetPosition(&pos);
            ...
            while (uint32 eventId = m_events.ExecuteEvent())
            {
                ...
                {
                    if (!me->isInCombat())
                        if (Creature* target = me->FindNearestCreature(42141, 30.0f, true)) // to test - used to be FindNearestCreatureOnTransportFloor - BfA function
                        {
                            if (me->GetTransport())
                            {
                                float dist = me->GetTransOffset().GetExactDist2d(&target->GetTransOffsetX, &target->GetTransOffsetY);
                                if (dist < 5.0f)
                                {
                ...
                case EVENT_TALK_PART_09: // spawn 3 gunpowder's
                {
                    ...
                    {
                        if (me->GetGUIDLow() == me->GetMap()->GameObject->GetGuidLow())
                        {
                ...
                {
                    if (Creature* orc = Unit::GetCreature(*me, m_bigOrcGUID))
                    {
 ...
 // ############################
        ...
        void SetGruntReactStateOnThisFloor(float floorZ, float range, ReactStates react)
        {
			...
			me->GetCreatureListWithEntryInGrid(cList, NPC_GUNSHIP_GRUNT, range);
        ...
        void SetRopeSelectableFlag(bool value)
        {
			...
			me->GetGameObjectListWithEntryInGrid(goList, GO_ROPE, 30.0f); // there are 2 rope on bridge
            for (std::list<GameObject*>::const_iterator itr = goList.begin(); itr != goList.end(); ++itr)
            {
                if (value)
                    (*itr)->RemoveFlag(UNIT_FIELD_FLAGS, GO_FLAG_NOT_SELECTABLE);
                else
                    (*itr)->SetFlag(UNIT_FIELD_FLAGS, GO_FLAG_NOT_SELECTABLE);
            }
        }
        ...
        std::vector<Creature*> FindJumpTargetPositions(float floorZ)
        {
			...
			me->GetCreatureListWithEntryInGrid(tList, NPC_GILNEAS_VEHICLE_ROPE, 75.0f);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = m_events.ExecuteEvent())
            {
                ...
                case EVENT_CHECK_ATTACK:
                {
                    if (!me->isInCombat())
                    {
                        if (Creature* target = me->FindNearestCreature(5.0f, true)) // hackfix used to be FindNearestAttackableCreatureOnTransportInFloor - a bfa function
                            me->Attack(target, true);
                    }
...
public:
   ...
   /* {
    ...
    Creature* FindRandomJumpTargetPosition(Player* player)
    {
        ...
        std::vector<Creature*> rList;
		std::list<Creature*> tList;
		player->GetCreatureListWithEntryInGrid(tList, NPC_GILNEAS_VEHICLE_ROPE, 50.0f);
        for (std::list<Creature*>::iterator itr = tList.begin(); itr != tList.end(); ++itr)
            if (Creature* target = (*itr))
       ...
       // {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = m_events.ExecuteEvent())
            {
                ...
                {
                    if (Unit::GetPlayer(*me, m_playerGUID)) // now we are outside the ship
