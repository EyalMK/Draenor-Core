...
/// <<<<<<<<<<<<<<<<<<<<<<<<<<<< CTRL+K then CTRL+U to uncomment the whole file.
//#include <ScriptPCH.h>
//#include "Cell.h"
//#include "CellImpl.h"
//#include "Creature.h"
...
//					m_emote = RAND(EMOTE_STATE_COWER, EMOTE_STATE_TALK, EMOTE_ONESHOT_CRY, EMOTE_STATE_SPELLPRECAST, EMOTE_STATE_EXCLAIM);
//					m_say = urand(0, 2);
//					if (Creature* npc = Unit::GetCreature(*me, m_citizenGUID))
...
//				me->GetPlayerListInGrid(PlayersInRange, 15.0f);
//				for (std::list<Player*>::const_iterator itr = PlayersInRange.begin(); itr != PlayersInRange.end(); ++itr)
//				{
//					if (cdList.find((*itr)) == cdList.end())
//						if ((*itr)->GetQuestStatus(QUEST_LOCKDOWN) != QUEST_STATUS_REWARDED)
//							cdList.insert(std::make_pair((*itr), 70));
//				}
//
//		}
//
...
//				{
//					for (std::map<Player*, int32>::iterator itr = cdList.begin(); itr != cdList.end(); ++itr)
//						if ((itr)->second > 0)
//							--(itr)->second;
//
//					if (!m_playerGUID)
//						for (std::map<Player*, int32>::iterator itr = cdList.begin(); itr != cdList.end(); ++itr)
//							if ((itr)->second == 0)
//								if (Player* player = Unit::GetPlayer(*me, (itr)->first->GetGUID()))
...
//									}
//
//					for (std::map<Player*, int32>::iterator itr = cdList.begin(); itr != cdList.end();)
//					{
//						if (ToBeDeleted(itr))
...
//				{
//					if (Player* p_Player = Unit::GetPlayer(*me, m_playerGUID))
//						Talk(0);
...
//		}
//
//		bool ToBeDeleted(std::map<Player*, int32>::iterator itr)
//		{
//			if (itr->second < 0)
//				if (Player* player = Unit::GetPlayer(*me, (itr)->first->GetGUID()))
...
//			{
//				me->CastSpell(me, SpellIds::SPELL_ENRAGE_56646);
//				m_enrage = true;
...
//				{
//					if (Creature* liam = Unit::GetCreature(*me, m_liamGUID))
//					{
//
//						Position l_Pos;
//						liam->GetPosition(&l_Pos);
//						float angle = liam->GetAngle(l_Pos.GetPositionX(), l_Pos.GetPositionY());
//
//						me->GetMotionMaster()->MovePoint(MOVE_TO_PRINCE_LIAM, l_Pos, true);
//					}
//					break;
//				}
...
//					{
//						Position l_Pos;
//						go->GetPosition(&l_Pos);
//
//						me->GetMotionMaster()->MovePoint(MOVE_TO_DOOR, l_Pos);
//					}
//						
//					break;
//				}
...
//		{
//			if (summon->GetEntry() == CreatureIds::NPC_RAMPAGING_WORGEN_35660)
//			{
...
//				{
//					me->SummonCreature(CreatureIds::NPC_RAMPAGING_WORGEN_35660, -1491.9f, 1413.1f, 35.56f, 5.5f, TEMPSUMMON_TIMED_DESPAWN, 120000);
//					break;
//				}
//				}
//			}
...
//		std::list<ObjectGuid> m_worgenList;
//
//		void Initialize()
//		{
//
//			m_worgenList.clear();
//			m_worgenList.push_back(CreatureIds::NPC_RAMPAGING_WORGEN_34884);
//			m_worgenList.push_back(CreatureIds::NPC_BLOODFANG_WORGEN_35118);
//			m_worgenList.push_back(CreatureIds::NPC_RAMPAGING_WORGEN_35660);
...
//					}
//
//					if (Creature* worgen = me->FindNearestCreature(NPC_RAMPAGING_WORGEN_35660, 5.0f, false))
//					{
...
//						}
//					}
//					else if (Creature* worgen = me->FindNearestCreature(CreatureIds::NPC_RAMPAGING_WORGEN_34884, 5.0f, false))
//					{
//						if (!worgen->isInCombat())
//						{
//							me->Attack(worgen, true);
...
//						}
//					}
//					else if (Creature* worgen = me->FindNearestCreature(CreatureIds::NPC_BLOODFANG_WORGEN_35118, 5.0f, false))
//					{
//						if (!worgen->isInCombat())
//						{
//							me->Attack(worgen, true);
...
//			if (!m_enrage && me->GetHealthPct() < 90.0f)
//			{
//				me->CastSpell(me, SpellIds::SPELL_ENRAGE);
//				m_enrage = true;
//				m_events.ScheduleEvent(EVENT_ENRAGE_COOLDOWN, urand(121000, 150000));
//			}
//		}
...
//						me->GetMotionMaster()->MovePoint(MOVE_TO_PLAYER, l_Pos);
//					}
//						
//					break;
//				}
...
//				{
//					if (Creature* marker = me->FindNearestCreature(CreatureIds::NPC_GILNEAS_EVACUATION_FACING_MARKER_35830, 100.0f))
//					{
//						Position l_Pos;
//						marker->GetPosition(&l_Pos);
//						me->GetMotionMaster()->MovePoint(MOVE_TO_MARKER, l_Pos);
//					}
//					break;
//				}
...
//					me->GetMotionMaster()->MovePoint(MOVE_TO_END, l_Pos);
//					break;
//				}
//				}
//			}
...
//					Position l_Pos;
//					me->GetPosition(&l_Pos);
//					me->GetMotionMaster()->MovePoint(MOVE_TO_END, l_Pos, true);
//					break;
//				}
//				}
//			}
...
//		Position const Pos = { go->GetPositionX(), go->GetPositionY(), go->GetPositionZ() };
//		return Pos;
//	}
//
...
//		Position const Pos = { x, y, go->GetPositionZ() };
//		return Pos;
//	}
//};
//
...
//		void DamageTaken(Unit* who, uint32& damage)
//		{
//			if (me->HealthBelowPct(85) && who->ToCreature())
//			{
//				damage = 0;
//			}
//				
...
//					}
//
//					if (Creature* worgen = me->FindNearestCreature(CreatureIds::NPC_RAMPAGING_WORGEN_34884, 5.0f, true))
//					{
//						me->Attack(worgen, true);
//						worgen->Attack(me, true);
//						m_events.ScheduleEvent(EVENT_CHECK_SHOWFIGHT, 2500);
//						return;
//					}
//					else if (Creature* worgen = me->FindNearestCreature(CreatureIds::NPC_BLOODFANG_WORGEN_35118, 5.0f, true))
//					{
//						me->Attack(worgen, true);
//						worgen->Attack(me, true);
//						m_events.ScheduleEvent(EVENT_CHECK_SHOWFIGHT, 2500);
//						return;
//					}
//					else if (Creature* worgen = me->FindNearestCreature(CreatureIds::NPC_RAMPAGING_WORGEN_35660, 5.0f, true))
//					{
//						me->Attack(worgen, true);
//						worgen->Attack(me, true);
//						m_events.ScheduleEvent(EVENT_CHECK_SHOWFIGHT, 2500);
...
//				creature->CastSpell(player, SpellIds::SPELL_BY_THE_SKIN_ON_HIS_TEETH, true);
//			}
//		return true;
//	}
//
...
//		{
//			m_phase = 0;
//			JumpW1[0] = { -1643.91f, 1482.96f, 63.22f, 3.95f };
//			JumpW1[1] = { -1638.83f, 1478.07f, 65.36f, 3.84f };
//		
//			JumpW1[2] = { -1631.49f, 1475.02f, 65.64f, 3.84f };
//
//			LandingW1[0] = { -1677.39f, 1455.52f, 52.29f, 4.06f };
//			LandingW1[1] = { -1671.89f, 1449.27f, 52.29f, 3.84f };
//			LandingW1[2] = { -1664.82f, 1443.62f, 52.29f, 3.84f };
//
//			LandingW2[0] = { -1704.93f, 1469.07f, 52.29f, 5.34f };
//			LandingW2[1] = { -1698.90f, 1472.92f, 52.29f, 5.41f };
//			LandingW2[2] = { -1703.37f, 1470.66f, 52.29f, 5.49f };
//			LandingW2[3] = { -1700.37f, 1473.32f, 52.29f, 5.41f };
//		}
//
//		void DoAction(int32 action) override
//		{
...
//			{
//				m_phase = 3;
//				Position jump = { -1717.73f, 1486.27f, 57.23f, 5.45f };
...
//			{
//				m_phase = 3;
//				const Position jump = { -1717.73f, 1486.27f, 57.23f, 5.45f };
//				me->GetMotionMaster()->MovePoint(21, jump);
//				break;
//			}
//			case 11:
...
//			me->GetPlayerListInGrid(pList, 20.0f);
//
...
//				Player* player = (*itr);
//				bool isOutdoor;
//				player->GetMap()->GetAreaId(player->GetPositionX(), player->GetPositionY(), player->GetPositionZ());
//				if (isOutdoor)
//				{
//					pList.remove(player);
//					return true;
...
//			m_lornaGUID = 0;
//			m_events.Reset();
//			Position pos = { -1792.37f, 1427.35f, 12.46f, 3.152f };
...
//				{
//					if (Player* player = Unit::GetPlayer(*me, m_playerGUID))
//						Talk(SAY_JOSAIH_AVERY_TRIGGER); // Tell Player they have been bitten
...
//							badAvery->SetFacingTo(badAvery->GetAngle(player)); // Face Player
//							badAvery->CastSpell(player, SPELL_COSMETIC_COMBAT_ATTACK, true); // Do Cosmetic Attack
//							player->GetMotionMaster()->MoveKnockbackFrom(-1791.94f, 1427.29f, 22.0f, 8.0f);
...
//				case EVENTS_ANIM_2:
//				{
//					if (Creature* badAvery = Unit::GetCreature(*me, m_badAveryGUID))
...
//				{
//					if (Creature* badAvery = Unit::GetCreature(*me, m_badAveryGUID))
//						if (Creature* lorna = Unit::GetCreature(*me, m_lornaGUID))
...
//				player->RemovePet(pet, PET_SLOT_OTHER_PET, true);
...
//			{
//				me->TextEmote(-106, 0);
//				DoCast(me, SpellIds::SPELL_ENRAGE);
...
//
//					
//					if (Creature* worgen = me->FindNearestCreature(NPC_RAMPAGING_WORGEN_35660, 10.0f))
//					{
//						me->Attack(worgen, true);
//						worgen->Attack(me, true);
//						m_events.ScheduleEvent(EVENT_CHECK_SHOWFIGHT, 2500);
//						return;
//					} 
//					else if (Creature* worgen = me->FindNearestCreature(NPC_BLOODFANG_RIPPER_35505, 10.0f))
//					{
//						me->Attack(worgen, true);
//						worgen->Attack(me, true);
//						m_events.ScheduleEvent(EVENT_CHECK_SHOWFIGHT, 2500);
...
//					{
//						if (Creature* krennan = Unit::GetCreature(*me, m_krennanTreeGUID))
//							if (Player* player = Unit::GetPlayer(*me, m_playerGUID))
//							{
//								krennan->AI()->Talk(SAY_KRENNAN_TREE_HELP);
...
//			player->CastSpell(player, SpellIds::SPELL_GILNEAS_CANNON_CAMERA);
//			player->SaveToDB();
...
//				{
//					GetTalkMember();
//					if (Unit::GetPlayer(*me, m_playerGUID))
//						if (Unit::GetCreature(*me, m_kingGUID))
//							if (Unit::GetCreature(*me, m_princeGUID))
//							{
...
//				{
//					if (Player* player = Unit::GetPlayer(*me, m_playerGUID))
//						if (Creature* king = Unit::GetCreature(*me, m_kingGUID))
//							king->AI()->Talk(0);
...
//				{
//					if (Player* player = Unit::GetPlayer(*me, m_playerGUID))
//						if (Creature* prince = Unit::GetCreature(*me, m_princeGUID))
//							prince->AI()->Talk(0);
...
//				{
//					// Todo: Needs to be a SelectRandomPlayerInRange - a BfA core function
//					if (Player* player = me->FindNearestPlayer(40.0f, true))
//						
...
//
//					cList.push_back(me->FindNearestCreature(NPC_BLOODFANG_LURKER, 4.0f));
//
...
//			m_maxDamage = 1;
//
//			me->GetCreatureListInGrid(CreaturesInRange, 10.0f);
//
//			for (std::list<Creature*>::const_iterator itr = CreaturesInRange.begin(); itr != CreaturesInRange.end(); ++itr)
//			{
//				if ((*itr)->GetEntry() == CreatureIds::NPC_BLOODFANG_STALKER)
//				{
//					m_targetList.push_back((*itr)->GetEntry());
//				}
//			}
//		}
//
...
//
//					for (std::list<uint32>::const_iterator itr = m_targetList.begin(); itr != m_targetList.end(); ++itr) {
//						if (Creature* worgen = me->FindNearestCreature((*itr), 5.0f))
//						{
//							me->Attack(worgen, true);
//							worgen->Attack(me, true);
//							m_events.ScheduleEvent(EVENT_CHECK_SHOWFIGHT, 2500);
...
//			for (std::list<Creature*>::const_iterator itr = CreaturesInRange.begin(); itr != CreaturesInRange.end(); ++itr)
//			{
//				if ((*itr)->GetEntry() == CreatureIds::NPC_FRENZIED_STALKER)
//				{
//					m_targetList.push_back((*itr)->GetEntry());
//				}
//			}
...
//		}
//
//		uint64 FindTargetEntry(uint32 entry)
//		{
//			if (m_targetList.empty())
//				return 0;
//
...
//
//						for (std::list<uint32>::const_iterator itr = m_targetList.begin(); itr != m_targetList.end(); ++itr) {
//							if (Creature* worgen = me->FindNearestCreature((*itr), 10.0f))
//							{
//								me->Attack(worgen, true);
//								worgen->Attack(me, true);
//								return;
