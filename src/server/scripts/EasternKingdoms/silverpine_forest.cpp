...
public:
	...
	bool OnQuestAccept(Player* player, Creature* creature, Quest const* quest) override
	{
		...
		{
			creature->AI()->Talk(SAY_QUESTACCEPT);
		...
		void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
		{
			if (Unit* unit = GetTarget())
				if (Creature* fallenHuman = unit->ToCreature())
				{
					...
					fallenHuman->SetFloatValue(UNIT_FIELD_HOVER_HEIGHT, frand(1.8f, 2.2f));
				}
		}
		...
		void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = m_events.ExecuteEvent())
			{
				...
				case EVENT_CHECK_SHOWFIGHT:
				{
					...
					{
						Creature* creature = me->FindNearestCreature(itr, 5.0f, true);
						if (creature)
						{
							worgen = creature;
							break;
