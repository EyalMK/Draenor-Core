...
{
public:
	playerScript_westfall_phase_handler() : PlayerScript("playerScript_westfall_phase_handler") { }
	...
	void UpdatePhaseMask(Player* p_Player)
	{
		...
		if (p_NewAreaId == 109)
		{
			if (p_Player->GetQuestStatus(26215) == QUEST_STATUS_COMPLETE || QUEST_STATUS_REWARDED)
			{
				l_PhaseMask == 3; // Lou alive
			}

			if (p_Player->GetQuestStatus(26232) == QUEST_STATUS_COMPLETE || QUEST_STATUS_REWARDED)
			{
				l_PhaseMask == 5; // Lou dead
			}
...
public:
	...
	struct npc_thugAI : public ScriptedAI
	{
		...
		void StartEvent()
		{
			if (!bSummoned)
			{
				if (Creature* Thug1 = me->SummonCreature(NPC_THUG, -9859.670f, 1332.077f, 41.902f, 2.393914f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000))
					if (Creature* Thug2 = me->SummonCreature(NPC_THUG, -9862.5244f, 1332.0290f, 41.9033f, 0.7949f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000))
						if (Creature* Thug3 = me->SummonCreature(NPC_THUG, -9862.6425f, 1335.2863f, 41.9033f, 5.4608f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000))
							if (Creature* Thug4 = me->SummonCreature(NPC_THUG, -9859.4287f, 1335.3573f, 41.9033f, 3.9607f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000))
							{
								...
								if (Creature* Thug3 = me->GetCreature(*me, Thug3GUID))
									if (Creature* Thug4 = me->GetCreature(*me, Thug4GUID))
									{
										...
										case 11:
										{
											...
											// Something about sounds of shoots and yells missing here
...
public:
	...
	struct npc_horatioAI : public ScriptedAI
	{
		...
		void MoveInLineOfSight(Unit* who)
		{
			...
			if (who->ToPlayer()->GetQuestStatus(QUEST_HERO_WESTFALL1 || QUEST_HERO_WESTFALL2) == QUEST_STATUS_COMPLETE)
			{
...
public:
	...
	struct npc_crate_mineAI : public ScriptedAI
	{
		...
		void PartySummoned()
		{
			if (!bSummoned)
			{
				if (Creature* glubtok1 = me->SummonCreature(42492, -9844.08f, 1397.808f, 37.213f, 0.56f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 85000))
					...
					if (Creature* glubtok1 = me->GetCreature(*me, GlubtoklGUID))
						if (Creature* shadowy1 = me->GetCreature(*me, Shadowy1GUID))
						{
							...
							case 1:
							{
								...
								shadowy1->GetMotionMaster()->MovePoint(0, -9837.022f, 1401.637f, 37.843f);
								EntryTime = 7000;
								Phase++;
								break;
							}
		...
		void UpdateAI(const uint32 diff)
		{
			if (HungryTimer < diff)
			{
				if (Creature* stew = me->FindNearestCreature(NPC_STEW, 10.0f, true))
				{
					...
					case 3:
					{
						if (Unit* player = me->GetPlayer(*stew, stew->GetUInt64Value(UNIT_FIELD_SUMMONED_BY)))
...
public:
	...
	struct npc_shadowy_towerAI : public ScriptedAI
	{
		...
		void MoveInLineOfSight(Unit* who)
		{
			...
			if (who->HasAura(79528)) // Invis potion
			{
		...
		void StartEvent()
		{
			if (!bSumm)
			{
				...
				if (!bSumm2)
				{
					if (Creature* Helix1 = me->SummonCreature(42655, -11128.11f, 547.52f, 70.41f, 3.32f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000))
					{
						Helix1GUID = Helix1->GetGUID();
						bSumm2 = true;
					}
				}
				...
				if (!bSumm3)
				{
					if (Creature* HelixMount1 = me->SummonCreature(42654, -11128.11f, 547.52f, 70.41f, 3.32f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000))
					{
						HelixMount1GUID = HelixMount1->GetGUID();
						bSumm2 = true;
					}
				}

				if (bSumm1 && bSumm2 && bSumm3)
				{
					bSumm = true;
					SummonTimer = 2000;
				}
		...
		void UpdateAI(const uint32 diff)
		{
			if (SummonTimer < diff)
			{
				...
				{
					if (Player* player = me->GetPlayer(*me, PlayerGUID))
						if (Creature* Helix1 = me->GetCreature(*me, Helix1GUID))
							if (Creature* HelixMount1 = me->GetCreature(*me, HelixMount1GUID))
								if (Creature* Shadowy3 = me->GetCreature(*me, Shadowy3GUID))
								{
									switch (Phase)
									{
									case 0:
									{	Helix1->CastSpell(HelixMount1, 192056, true); // Ride vehicle
										Helix1->SetInFront(me);
										SummonTimer = 1000;
										Phase++;
										break;
									}
									...
									case 11:
									{
										if (!bExit)
										{
											...
											HelixMount1->ForcedDespawn(1000);
											Helix1->CastSpell(Shadowy3, 64446, true);
											Helix1->ForcedDespawn(1000);
											bExit = true;
										}
									}
									break;
												...
												if (Creature* Thief10 = me->SummonCreature(NPC_THIEF, -10504.77f, 1053.57f, 59.86f, 4.84f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 100000))
													if (Creature* Glubtok4 = me->SummonCreature(42755, -10506.12f, 1053.13f, 59.10f, 4.92f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 120000))
														if (Creature* Helix2 = me->SummonCreature(42753, -10509.368f, 1057.28f, 57.86f, 4.95f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 120000))
															if (Creature* HelixMount2 = me->SummonCreature(42654, -10509.368f, 1057.28f, 57.86f, 4.95f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 120000))
															{
											...
											if (Creature* Investigator = me->FindNearestCreature(NPC_INVESTIGATOR, 50.0f, true))
												if (Creature* Investigator2 = me->FindNearestCreature(NPC_INVESTIGATOR3, 50.0f, true))
												{
													...
													case 21:
													{
														...
														if (Creature* HelixMount2 = me->FindNearestCreature(42654, 30.0f, true))
														{
															HelixMount2->SetWalk(true);
															HelixMount2->GetMotionMaster()->MovePoint(7, -10520.01f, 1062.31f, 55.386f);
														}
													...
													case 22:
													{
														...
														if (Creature* HelixMount2 = me->FindNearestCreature(42654, 40.0f, true))
														{
															HelixMount2->ForcedDespawn();
														}
...
public:
	...
	struct npc_horatio_investigateAI : public ScriptedAI
	{
		...
		void MoveInLineOfSight(Unit* who)
		{
			...
			if (who->ToPlayer()->GetQuestStatus(QUEST_LOUS_PARTING_THOUGHTS) == QUEST_STATUS_COMPLETE)
			{
				if (who->IsWithinDistInMap(me, 15.0f))
				{
					who->SetPhaseMask(5, true);
					StartSummon();
					PlayerGUID = who->GetGUID();
				}
			}
