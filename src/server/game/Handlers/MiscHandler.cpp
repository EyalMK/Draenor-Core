...
void WorldSession::HandleChangePlayerDifficulty(WorldPacket& recvData)
{
	...
	uint32 difficulty = Difficulty10N;

	Player* player = GetPlayer();
	Map* map = player->FindMap();
	Group* group = player->GetGroup();

	Difficulty oldDifficulty = map->GetDifficultyID();

	switch (oldDifficulty)
	{
		case Difficulty10N:
			difficulty = Difficulty10HC;
			break;
		case Difficulty10HC:
			difficulty = Difficulty10N;
			break;
		case Difficulty25N:
			difficulty = Difficulty25HC;
			break;
		case Difficulty25HC:
	...
	// You must be in a raid group to enter a raid, unless the raid is from an older expansion or your account has GM level.
	if (!player->isGameMaster() && uint32(player->GetExpByLevel()) <= map->Expansion() && (!group || !group->isRaidGroup()))
		return;
	...
	if (!player->isGameMaster() && !player->Satisfy(sObjectMgr->GetAccessRequirement(map->GetId(), Difficulty(difficulty)), map->GetId(), true))
		return;
	...
	if (Difficulty(difficulty) == map->GetDifficultyID() && (map->GetDifficultyID() == Difficulty10HC || map->GetDifficultyID() == Difficulty25HC))
	...
	if (result != DIFF_CHANGE_FAIL_COOLDOWN && result != DIFF_CHANGE_FAIL_ALREADY_HEROIC)
	{
			...
			if (group->IsLeader(player->GetGUID()))
			{
				for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
				{
					if (Player* groupGuy = itr->getSource())
					{
						...
						else if (groupGuy->GetMap()->IsRaid() && (difficulty == Difficulty10HC || difficulty == Difficulty25HC) &&
							groupGuy->GetBoundInstance(groupGuy->GetMapId(), Difficulty(difficulty)) && player->GetBoundInstance(groupGuy->GetMapId(), Difficulty(difficulty)) &&
							groupGuy->GetBoundInstance(groupGuy->GetMapId(), Difficulty(difficulty))->save != player->GetBoundInstance(groupGuy->GetMapId(), Difficulty(difficulty))->save)
						{
		...
		{
			if (player->GetMap()->IsRaid() && player->GetInstanceScript() && player->GetInstanceScript()->IsEncounterInProgress())
				result = DIFF_CHANGE_FAIL_ENCOUNTER_IN_PROGRESS;             // A Boss encounter is in progress.
			else if (player->GetMap()->IsRaid() && player->GetInstanceScript() && !player->GetInstanceScript()->IsEncounterInProgress() && player->isInCombat())
				result = DIFF_CHANGE_FAIL_SOMEONE_IN_COMBAT;                 // Is in combat with mobs or something.
			else if (player->GetMap()->IsRaid() && player->GetInstanceScript() && !player->GetInstanceScript()->IsEncounterInProgress() && (now - player->GetLastCombatTime()) < 5 * MINUTE)
			{
				groupCombatCooldown = true;
				lastCombatTime = uint32(now - player->GetLastCombatTime());
				result = DIFF_CHANGE_FAIL_COOLDOWN;                          // Was in combat recently.
			}
			else if (player->GetSession()->isLogingOut() || player->IsNonMeleeSpellCasted(false))
	...
	if (result == DIFF_CHANGE_SUCCESS)  // It finally worked! Update everything.
	{
		...
		if (group)
		{
			if (group->IsLeader(player->GetGUID()))
			{
				...
				if (!player->HasDynamicDifficultyMap(map->GetId()))
					player->AddDynamicDifficultyMap(map->GetId());

				// Set the leader as being on a DynamicDifficultyMap if he is not.
				if (!player->IsOnDynamicDifficultyMap())
					player->SetOnDynamicDifficultyMap(true);
				...
				for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next()) // Remove the loading screen and update the zone for all players here.
				{
					if (Player* groupGuy = itr->getSource())
					{
						...
						if (time_t timeReset = uint64(sWorld->getWorldState(WS_WEEKLY_QUEST_RESET_TIME)))
						{
							...
							groupGuy->SendRaidInstanceMessage(map->GetId(), Difficulty(difficulty), timeleft);
						}
		...
		{
			// Set raid difficulty.
			player->SetRaidDifficultyID(Difficulty(difficulty));
			...
			if (!player->GetMap()->AddPlayerToMap(player))
				sLog->outError(LOG_FILTER_NETWORKIO, "Failed to change dynamic difficulty for player %s (%d) to map %s because of unknown reason!", player->GetName(), player->GetGUIDLow(), map->GetMapName());
			...
			{
				uint32 timeleft = uint32(timeReset - time(NULL));
				player->SendRaidInstanceMessage(map->GetId(), oldDifficulty, timeleft);
			}
