...
void WorldSession::HandleLfgJoinOpcode(WorldPackets::LFG::DFJoin& dfJoin)
{
    ...
    TC_LOG_DEBUG("lfg", "CMSG_DF_JOIN %s roles: %u, Dungeons: %u, Comment: %s", GetPlayerInfo().c_str(), dfJoin.Roles, uint8(newDungeons.size()), dfJoin.Comment.c_str());

    sLFGMgr->JoinLfg(GetPlayer(), uint8(dfJoin.Roles), newDungeons, dfJoin.Comment);
}
...
{
    TC_LOG_DEBUG("lfg", "CMSG_DF_PROPOSAL_RESPONSE %s proposal: %u accept: %u",
...
void WorldSession::SendLfgPlayerLockInfo()
{
    ...
    for (auto const& lock : sLFGMgr->GetLockedDungeons(_player->GetGUID()))
        lfgPlayerInfo.BlackList.Slots.emplace_back(lock.first, lock.second.lockStatus, lock.second.requiredItemLevel, lock.second.currentItemLevel);
...
void WorldSession::SendLfgPartyLockInfo()
{
    ...
    for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
    {
        ...
        lfgPartyInfo.Players.emplace_back();
        WorldPackets::LFG::LFGBlackList lfgBlackList = lfgPartyInfo.Players.back();
        lfgBlackList.PlayerGuid = pguid;
        for (auto const& lock : sLFGMgr->GetLockedDungeons(pguid))
            lfgBlackList.Slots.emplace_back(lock.first, lock.second.lockStatus, lock.second.requiredItemLevel, lock.second.currentItemLevel);
    }
...
void WorldSession::HandleDFGetJoinStatus(WorldPackets::LFG::DFGetJoinStatus& dfGetJoinStatus)
{
    ...
    SendLfgUpdateStatus(updateData, GetPlayer()->GetGroup() != NULL);
}
...
void WorldSession::SendLfgUpdateStatus(lfg::LfgUpdateData const& updateData, bool party)
{
    ...
    {
        case lfg::LFG_UPDATETYPE_JOIN_QUEUE_INITIAL:    // Joined queue outside dungeon
            join = true;
            lfgJoined = true;
            break;
    ...
    lfgUpdateStatus.Ticket.RequesterGuid = _player->GetGUID();
    lfgUpdateStatus.Ticket.Id = queueId;
    lfgUpdateStatus.Ticket.Type = WorldPackets::LFG::RideType::Lfg;
    lfgUpdateStatus.Ticket.Time = joinTime;
    ...
    lfgUpdateStatus.RequestedRoles = uint32(sLFGMgr->GetRoles(_player->GetGUID()));
    lfgUpdateStatus.IsParty = party;
    lfgUpdateStatus.NotifyUI = true;
    lfgUpdateStatus.Joined = join;
    lfgUpdateStatus.LfgJoined = lfgJoined;
...
{
    TC_LOG_DEBUG("lfg", "SMSG_ROLE_CHOSEN %s guid: %s roles: %u",
...
void WorldSession::SendLfgRoleCheckUpdate(lfg::LfgRoleCheck const& roleCheck)
{
    ...
    if (!roleCheck.roles.empty())
    {
        ...
        for (lfg::LfgRolesMap::const_iterator it = roleCheck.roles.begin(); it != roleCheck.roles.end(); ++it)
        {
            ...
            roles = it->second;
            lfgRoleCheckUpdate.Members.emplace_back(it->first, roles, ASSERT_NOTNULL(ObjectAccessor::FindConnectedPlayer(it->first))->getLevel(), roles > 0);
        }
    }
...
void WorldSession::SendLfgJoinResult(lfg::LfgJoinResultData const& joinData)
{
    ...
    lfgJoinResult.Ticket.RequesterGuid = _player->GetGUID();
    lfgJoinResult.Ticket.Id = sLFGMgr->GetQueueId(_player->GetGUID());
    lfgJoinResult.Ticket.Type = WorldPackets::LFG::RideType::Lfg;
    lfgJoinResult.Ticket.Time = getMSTime();
    ...
    for (lfg::LfgLockPartyMap::const_iterator it = joinData.lockmap.begin(); it != joinData.lockmap.end(); ++it)
    {
        ...
        for (lfg::LfgLockMap::const_iterator itr = it->second.begin(); itr != it->second.end(); ++itr)
        {
            TC_LOG_TRACE("lfg", "SendLfgJoinResult:: %s DungeonID: %u Lock status: %u Required itemLevel: %u Current itemLevel: %f",
                _player->GetGUID().ToString().c_str(), (itr->first & 0x00FFFFFF), itr->second.lockStatus, itr->second.requiredItemLevel, itr->second.currentItemLevel);
...
{
    // TODO: Look into why Selected Dungeons work fine, but the RDF Status does not update appropriately.
    ...
    TC_LOG_DEBUG("lfg", "SMSG_LFG_QUEUE_STATUS %s state: %s, dungeon: %u, waitTime: %d, "
        ...
        "queuedTime: %u, tanks: %u, healers: %u, dps: %u, joinTime: %d, queueId: %u",
        ...
        queueData.queuedTime, queueData.tanks, queueData.healers, queueData.dps, int32(joinTime), queueId);

    WorldPackets::LFG::LfgQueueStatus lfgQueueStatus;
    lfgQueueStatus.Ticket.RequesterGuid = _player->GetGUID();
    lfgQueueStatus.Ticket.Id = queueId;
    lfgQueueStatus.Ticket.Type = WorldPackets::LFG::RideType::Lfg;
    lfgQueueStatus.Ticket.Time = joinTime;
...
void WorldSession::SendLfgPlayerReward(lfg::LfgPlayerRewardData const& rewardData)
{
    ...
    for (uint8 i = 0; i < QUEST_REWARD_ITEM_COUNT; ++i)
        if (uint32 currencyId = rewardData.quest->RewardCurrencyId[i])
            lfgPlayerReward.Rewards.emplace_back(currencyId, rewardData.quest->RewardCurrencyCount[i], 0, true);
...
void WorldSession::SendLfgBootProposalUpdate(lfg::LfgPlayerBoot const& boot)
{
    ...
    lfgBootPlayer.VoteInProgress = boot.inProgress;
    lfgBootPlayer.VotePassed = agreeNum >= lfg::LFG_GROUP_KICK_VOTES_NEEDED;
    lfgBootPlayer.MyVoteCompleted = playerVote != lfg::LFG_ANSWER_PENDING;
    lfgBootPlayer.MyVote = playerVote == lfg::LFG_ANSWER_AGREE;
    lfgBootPlayer.Target = boot.victim;
    lfgBootPlayer.TotalVotes = votesNum;
    lfgBootPlayer.BootVotes = agreeNum;
    lfgBootPlayer.TimeLeft = secsleft;
    lfgBootPlayer.VotesNeeded = lfg::LFG_GROUP_KICK_VOTES_NEEDED;
    lfgBootPlayer.Reason = boot.reason;
...
void WorldSession::SendLfgUpdateProposal(lfg::LfgProposal const& proposal)
{
    ...
    lfgProposalUpdate.Ticket.RequesterGuid = _player->GetGUID();
    lfgProposalUpdate.Ticket.Id = queueId;
    lfgProposalUpdate.Ticket.Type = WorldPackets::LFG::RideType::Lfg;
    lfgProposalUpdate.Ticket.Time = getMSTime();
...
void WorldSession::SendLfgTeleportError(lfg::LfgTeleportError error)
{
    TC_LOG_DEBUG("lfg", "SMSG_LFG_TELEPORT_DENIED %s reason: %u",
        GetPlayerInfo().c_str(), error);
    SendPacket(WorldPackets::LFG::LfgTeleportDenied(error).Write());
}
