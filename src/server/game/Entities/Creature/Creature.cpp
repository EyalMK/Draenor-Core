...
// Quest for weekly looting, new MOP and WoD Lockout system.
bool Creature::HasWeeklyBossLootQuestId()
{
	uint32 questId = sObjectMgr->GetWeeklyBossLootQuestId(GetEntry(), GetMap()->IsRaid() ? GetMap()->GetDifficultyID() : DifficultyNone);
	if (!questId)
		return false;
