-- Remove Goblin Commoners
DELETE FROM `creature` WHERE `id`=20102 AND `zoneId`=5287 AND `areaId`=35 AND `guid` NOT IN (93852, 93858);

-- Remove duplicate Landro Longshot
DELETE FROM `creature` WHERE `guid`=6775;

-- Remove wandering for McGavan
UPDATE `creature` SET `spawndist`=0, `MovementType`=0 WHERE `id`=7794;

-- Remove wandering for Sprogger
UPDATE `creature` SET `spawndist`=0, `MovementType`=0 WHERE `id`=2594;
